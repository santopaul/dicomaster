...
def show_banner(args):
    ...
    try:
        if _BANNER_AVAILABLE:
            ...
            banner_text = pyfiglet.figlet_format('DicomTool', font='standard')
            colored_banner = colored(banner_text, 'green')
            name = colored('Dr. Santo Paul', 'yellow', attrs=['bold'])
            print(colored_banner)
            print(colored(f'v{__version__} — Secure DICOM Anonymizer & Batch Processor by {name}', 'cyan', attrs=['bold']))
            return
    except Exception as e:
        logging.debug(f"Banner error (non-fatal): {e}")
    # Simple fallback banner
    name = 'Dr. Santo Paul'
    print(f"*** DicomTool v{__version__} — Secure DICOM Anonymizer & Batch Processor by {name} ***")
...
def process_and_save(path: str, args, outputs_map: Dict[str, List[str]], run_mapping: Dict[str, Any], dry_run: bool=False, suppress_details: bool=False) -> Optional[Dict[str, Any]]:
    pathp = Path(path)
    try:
    ...
    if 'html' in outputs_map:
        ...
        for t in targets:
            ...
            if args.dry_run:
                ...
                try:
                    generate_html_report({'STAT_Report': stat, 'Full_Report': full, 'Private_Tags': sanitized.get('private_tags', [])}, thumb_path, str(dest))
    ...
    if 'report' in outputs_map:
        ...
        for t in targets:
            ...
            if args.dry_run:
                ...
                try:
                    generate_metadata_image({'STAT_Report': stat, 'Full_Report': full, 'Private_Tags': sanitized.get('private_tags', [])}, thumb_path, str(dest))
